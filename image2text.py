...
src_embeddings, src_id2word, src_word2id, source_vocab = load_vec(source_embedding_path, nmax=200000)
...
class Seq2Seq:
    ...
    def __init__(self):
        ...
        with tf.variable_scope('rnn_i/o'):
            ...
            self.decoder_inputs = tf.placeholder(tf.int32, shape=[self.decoder_length, self.batch_size])  #-tokens

            self.source_sequence_length = tf.placeholder(tf.int32, shape=[self.batch_size])  # i.e. self.decoder_length use to limit training iter while unrolling .. nneds to be corrected
        ...
        with tf.variable_scope('decoder'):
            ...
            self.train_helper = TrainingHelper(inputs=self.decoder_input_embeddings,
                                               ...
                                               time_major=False)  # need to calculate source sequence length what is this what does
