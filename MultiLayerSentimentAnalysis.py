...
class SentimentAnalysisMultiLayerLSTM:
    ...
    def __init__(self):
        ...
        with tf.variable_scope('rnn_cell'):
            self.initial_state, self.lstm_outputs, self.final_state, self.cell = self.build_lstm_layers(self.lstm_sizes,self.inputs,self.keep_prob,self.batch_size)
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
      ...
      for idx in range(0, n_train,BATCH_SIZE):
        ...
        BATCH_X, BATCH_Y=get_training_batch_twets(train_data[idx:(idx+BATCH_SIZE)], BATCH_SIZE, model.embedding_dim, num_classes=model.num_classes, maxlen=model.max_sequence_length)
        loss_batch, accuracy_batch, state = model.train(BATCH_X, BATCH_Y, state)
        loss_train += loss_batch
        accuracy_train += accuracy_batch
        print("EPOCH: " + str(epoch) + "BATCH_INDEX:" + str(idx) + "Batch Loss:" + str(loss_batch) + "Batch Accuracy:" + str(accuracy_train)  )
